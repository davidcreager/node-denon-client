...
class DenonClient extends Connection {
  ...
  constructor(host,port=23) {
    ...
    this.regExTable = {
      ...
      'zone2': {
        ...
        /**
         * @event zone2Changed
         * @param {Zone2Options} zone2Status The current zone 2 status
         */
        emit: 'zone2Changed'
      },
      'videoSelect': {
        regEx: DenonRegex.constructStatusChangedRegex(Options.VideoSelectOptions),
        /**
         * @event videoSelectChanged
         * @param {VideoSelectOptions} videoSelectStatus The current video select status
         */
        emit: 'videoSelectChanged'
      }
    };
  ...
  /**
   * Sets the video select mode of the Denon AVR. (Video Input).
   * Use {VideoSelectOptions}.
   *
   * @method setVideoSelectMode
   * @param {VideoSelectOptions} videoSelectOptions [The video select option]
   * @return {Promise} [A response]
   */
  setVideoSelectMode(videoSelectOptions) {

    return this.sendCommand('SV', videoSelectOptions,
      this.getEvent('videoSelect'));
  }

  /**
   * Returns the current video select mode status of the Denon AVR.
   *
   * @method getVideoSelectMode
   * @return {Promise} [A response]
   */
  getVideoSelectMode() {

    return this.sendCommand('SV', Options.VideoSelectOptions.Status,
      this.getEvent('videoSelect'));
  }

  /**
   * Cancels the Video Select state of the Denon AVR.
   *
   * @method cancelVideoSelectMode
   * @return {Promise} [A response]
   */
  cancelVideoSelectMode() {

    // return this.sendCommand('VS', Options.VideoSelectOptions.Cancel,
    //   this.getEvent('videoSelect'));
    return this.setVideoSelectMode(Options.VideoSelectOptions.Cancel);
  }
